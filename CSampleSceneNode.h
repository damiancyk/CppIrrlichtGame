...
public:
	...
	CSampleSceneNode(scene::ISceneNode* parent, scene::ISceneManager* mgr, s32 id, core::vector3df pos)
		: scene::ISceneNode(parent, mgr, id, pos)
	{
        ...
        for(int i=0;i<4;i++)Vertices[i].Pos+=pos;
	...
	*/
		...
		for (s32 i=1; i<4; ++i)
			Box.addInternalPoint(Vertices[i].Pos+pos);
	}
